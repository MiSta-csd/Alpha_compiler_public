...
{
       0,   139,   139,   140,   144,   161,   165,   175,   178,   181,
     184,   190,   201,   212,   215,   218,   225,   228,   233,   238,
     243,   248,   253,   262,   271,   280,   289,   294,   299,   299,
     312,   312,   324,   329,   336,   340,   350,   360,   379,   399,
     418,   438,   443,   472,   477,   481,   485,   491,   497,   517,
     536,   548,   554,   559,   572,   577,   583,   593,   597,   608,
     617,   620,   625,   633,   640,   647,   653,   659,   675,   693,
     701,   709,   721,   721,   731,   736,   773,   780,   801,   811,
     819,   839,   842,   845,   848,   851,   854,   859,   872,   889,
     896,   915,   922,   929,   947,   949,   963,   979,   994,  1012,
    1019,  1029
};
#endif
...
#line 350 "parser.y"
											...
											if((yyvsp[0].expr_p)) {
												(yyval.expr_p) = true_test((yyvsp[0].expr_p));// TODO make it BOOLEXPR_E
												std::vector<int> *tmp = (yyval.expr_p)->truelist;
												(yyval.expr_p)->truelist = (yyval.expr_p)->falselist;
												(yyval.expr_p)->falselist = tmp;
												(yyval.expr_p)->type = BOOLEXPR_E;
											}
										}
...
#line 443 "parser.y"
		   							 		...
		   							 		if((yyvsp[-2].expr_p) && !member_flag && (yyvsp[-2].expr_p)->sym && ((yyvsp[-2].expr_p)->sym->type==LIB_FUNC || (yyvsp[-2].expr_p)->sym->type==USER_FUNC) ) {
												yyerror("invalid assignment (lvalue is a function)");
											}else {
...
#line 820 "parser.y"
											...
											if(!hasError) {
												...
												(yyvsp[-2].st_entryVal)->totalLocals = currscopeoffset();
...
#line 896 "parser.y"
											...
											if((yyvsp[-1].expr_p)) {
												...
												(yyvsp[-1].expr_p)->falselist = new std::vector<int>();
